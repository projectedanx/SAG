...
const AppCardHeader: React.FC<{ app: AppItem, isSelected?: boolean }> = ({ app, isSelected }) => {
...
const AppCardMetrics: React.FC<{
  ...
  const handleFeedback = (id: string, feedback: 'Resonant' | 'Dissonant' | undefined) => {
      if (onHumanFeedback) {
          onHumanFeedback(id, feedback);
      } else {
          setHumanFeedback(id, feedback);
      }
  };
        ...
        <div className="mt-3 pt-3 border-t border-slate-800 flex flex-col gap-2 bg-slate-900/50 p-2 rounded">
          ...
          <div className="flex gap-2">
            <button
              onClick={(e) => {
                e.stopPropagation();
                handleFeedback(app.id, app.humanFeedback === 'Resonant' ? undefined : 'Resonant');
              }}
            ...
            <button
              onClick={(e) => {
                e.stopPropagation();
                handleFeedback(app.id, app.humanFeedback === 'Dissonant' ? undefined : 'Dissonant');
              }}
              className={`flex-1 py-1 px-2 rounded flex items-center justify-center gap-1 text-xs transition-colors ${
...
const AppCardActions: React.FC<{
  ...
  onAudit?: (id: string) => void,
  onSculpt?: (id: string, friction: string) => void
}> = ({ app, onGenerate, onGenerateSpecs, onViewSpecs, onAudit, onSculpt }) => {
  return (
...
export const AppCard: React.FC<AppCardProps> = ({ app, isSelected, onGenerate, onGenerateSpecs, onViewSpecs, onAudit, onSculpt, onSelect, onHumanFeedback }) => {
  ...
  return (
    ...
    >
      <AppCardHeader app={app} isSelected={isSelected} />
      <AppCardMetrics app={app} onHumanFeedback={onHumanFeedback} />
      <AppCardActions
        app={app}
        onGenerate={onGenerate}
        onGenerateSpecs={onGenerateSpecs}
